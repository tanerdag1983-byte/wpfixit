...
Deployment and authenticated live acceptance: complete (commit f68fe5a deployed by Render as dep-d9l90t0ae00c7384l1m0 and by Vercel as dpl_951LcC7x6KKHHPi5YNX8rfhp6; fixed production alias returned HTTP 200; authenticated staging sync exercised consecutive 0/50/100 windows with 133 rendered unique opportunities, zero duplicate keywords, and 50 current-run Nieuw labels; a temporary invalid DataForSEO login produced the expected safe failure while all 133 unique rows remained visible, the original login was restored without changing the password, Connection successful was reconfirmed, and the recovered offset-150 window added 50 new rows with 180 unique visible opportunities and no duplicate keyword; a second controlled failure was observed directly in Supabase with next_offset unchanged at 200, after which the original login was restored and Connection successful reconfirmed).
