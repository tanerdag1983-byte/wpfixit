      ...
      {items.length === 0 ? (
        ...
        <div className="priority-table">
          ...
          {items.map((item) => {
            ...
            const searchDisplay =
              item.impressions && item.ctr
                ? `${item.impressions.toLocaleString()} impressies · ${(item.ctr * 100).toFixed(1)}% CTR`
                : item.impressions
                  ? `${item.impressions.toLocaleString()} impressies`
                  : item.average_position
                    ? `positie ${item.average_position.toFixed(1)}`
                    : "Geen data";
            const trafficDisplay =
              item.sessions || item.conversions
                ? `${item.sessions ?? 0} sessies${item.conversions ? ` · ${item.conversions} conversies` : ""}`
                : "Geen data";
