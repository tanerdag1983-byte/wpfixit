...
describe("WordPressBridgePanel", () => {
  beforeEach(() => {
    ...
    apiRequest.mockImplementation((path: string, init?: RequestInit) => {
      ...
      if (path.endsWith("/wordpress-outbound-credential")) {
        if (init?.method === "POST") {
  ...
  it("shows a newly created project key only once", async () => {
    ...
    expect(await screen.findByLabelText("Eenmalige projectkey")).toHaveValue(
      "wpfx_once",
    );
    expect(screen.getByText("Deze key wordt niet opnieuw getoond.")).toBeVisible();
    expect(apiRequest).toHaveBeenCalledWith(
      "/projects/project-1/wordpress-outbound-credential",
      {
        method: "POST",
